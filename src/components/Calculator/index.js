...
const Calculator = () => {
  ...
  const { response } = useFetch(`https://vmi417694.contaboserver.net/api/cambiodivisa/conversion`, null, "get");
  let pricesEl;
  ...
  const changeConditionHandler = async (oldCondition, setFieldValue) => {
    await setFieldValue("condition", oldCondition === "buying" ? "selling" : "buying");
  ...
  const changeAmountHandler = (e, name) => {
    ...
    if (condition === "selling") {
      fieldValue = name === "sending" ? +e.target.rawValue * buying : +e.target.rawValue / buying;
      calculatorRef.current.setFieldValue(fieldName, fieldValue);
    } else {
      fieldValue = name === "sending" ? +e.target.rawValue / selling : +e.target.rawValue * selling;
      calculatorRef.current.setFieldValue(fieldName, fieldValue);
    }
  };
        ...
        {({ isValid, setFieldValue, values, errors, isSubmitting }) => (
          <Form className={classes.CalculatorForm}>
            <Input
              ...
              selling={selling}
              buying={buying}
              selected={() => changeConditionHandler(values.condition, setFieldValue)}
            ...
            {errors.sending || errors.receiving ? (
              <span className={classes.ErrorMessage}>* {errors.sending || errors.receiving}</span>
            ) : null}
            <SavingPrice
              condition={values.condition}
              selling={selling}
            ...
            <div className={classes.CalculatorInfo}>
              ...
              <button
                ...
                disabled={!isValid || isSubmitting || !response || (!selling && !buying)}
